...
class zStyle(TreeBase):
    ...
    def getStyleNodes(self):
        ...
        for node in self.root:
            if node.tag in ("presets", "sorted", "depends"):
                continue
            for zstyle in node.findall("style"):
                item = StyleItem(zstyle, node)
    ...
    def getStyleNode(self, key, value):
        zstyles = self.getStyleNodes()
        for zstyle in zstyles:
            n = zstyle.getName()
            v = zstyle.getValue()
    ...
    def getSkinNode(self, key, value):
        zstyle = self.getStyleNode(key, value)
        if zstyle:
            return zstyle.getSkinNode()
        else:
    ...
    def getGroup(self):
        ...
        zstyles = self.getStyleNodes()
        for zstyle in zstyles:
            if self.checkDependency is not None and not self.checkDependency(zstyle.getDepend()):
                continue
            p = zstyle.getParentName()
            n = zstyle.getName()
            v = zstyle.getValue()
    ...
    def getPreset(self):
        ...
        d = dict()
        for node in self.root.findall("presets"):
            for zstyles in node.findall("style"):
                p = zstyles.attrib.get("name", "")
                n = zstyles.attrib.get("value", "")
                if p not in d:  # .has_key(p):
                    d[p] = dict()
                if n not in d[p]:  # .has_key(n):
                    d[p][n] = dict()
            ...
            for zstyles in node.findall("style"):
                for s in zstyles:
                    ...
                    if n == name and v == value:
                        if ("preview") in zstyles.attrib:  # .has_key("preview"):
                            return zstyles.attrib.get("preview", "")
                        return default
        return default
    ...
    def getPreview(self, name, value, default=None):
        ...
        for node in self.root:
            for zstyles in node.findall("style"):
                n = zstyles.attrib.get("name", "")
                v = zstyles.attrib.get("value", "")
                if n == name and v == value:
                    if ("preview") in zstyles.attrib:  # .has_key("preview"):
                        return zstyles.attrib.get("preview", "")
                    return default
    ...
    def getDepends(self):
        ...
        if not self.hasStyle():
            return d
        for node in self.root:
            for zstyles in node.findall("depend"):
                n = zstyles.attrib.get("name", "")
                v = zstyles.attrib.get("value", "")
                if n and v:
                    d[n] = v
    ...
    def getSkinComponents(self, config_Styles):
        ...
        for key in sorted(config_Styles.zstyle):
            value = config_Styles.zstyle[key].getValue()
...
class zStyleUser():
    ...
    def loadToStyle(self, zstyle, skin_name):
        if zstyle.root is None:
            zstyle.root = ElementTree.fromstring("<styles></styles>")
        ...
        for node in self.user_root:
            ...
            self.__addNode(zstyle, node)

    def __addNode(self, zstyle, node):
        for elem in zstyle.root:
            if elem.tag == node.tag:
                for item in node:
                    elem.append(item)
                return
        zstyle.root.append(node)
