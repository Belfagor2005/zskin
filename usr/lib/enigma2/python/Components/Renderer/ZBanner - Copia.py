...
class ZBanner(Renderer):
    ...
    def showBackdrop(self):
        if self.instance:
            ...
            # this old metod work only if change channel with right or left button and put info button
            # if os.path.exists('/var/lib/dpkg/info'):
                # self.instance.setPixmap(gPixmapPtr())  # CVS
            # else:
                # self.instance.setPixmap(None)  # OPEN
            # # size = self.instance.size()
            ...
            # self.picload.setPara((size.width(),size.height(),sc[1],sc[1],0,1,'FF000000'))
    ...
    def savePoster(self):
        ...
        with open(self.pstrNm, 'wb') as f:
            ...
            f.close()
        if os.path.exists(self.pstrNm):
            print('save poster backdrop show poster')
            self.showBackdrop()
        return
